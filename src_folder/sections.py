...
from PyQt5.QtGui import QPainter, QPen, QStandardItemModel, QBrush
from PyQt5.QtWidgets import QFrame, QComboBox, QScrollArea, QGraphicsView, QGraphicsScene, QGraphicsItem, \
    QGraphicsPixmapItem

# Relative files
from src_folder.utils import draw_continuity_labels, draw_pipelines, draw_junctions, draw_equipments, draw_area_breaks, \
...
class FrameDrawing(QFrame):
    def paintEvent(self, e):  # override paintEvent() to draw what we want
        ...
        if 'Equipment Tags' in p_Widget.selectedData:
            draw_tag(self, p_Widget.selectedData['Equipment Tags'], minX, minY, maxX, maxY, selected_location, selected_part,et_column_data, color=Qt.darkGreen)
        if 'Sensors' in p_Widget.selectedData:
            draw_sensors(self, p_Widget.selectedData['Sensors'], minX, minY, maxX, maxY, selected_location, selected_part,ss_column_data, color=Qt.darkMagenta)
...
class FrameDrawing1(QGraphicsView):

    def __init__(self, parent):
        super(FrameDrawing1, self).__init__(parent)
        self._zoom = 0
        self._empty = True
    ...
    def paintEvent(self, e):  # override paintEvent() to draw what we want
        ...
        if 'Equipment symbols' in p_Widget.selectedData:
            draw_equipments(self, p_Widget.selectedData['Equipment symbols'], minX, minY, maxX, maxY, selected_location, selected_part,es_column_data, color=Qt.darkYellow)
        if 'Equipment Tags' in p_Widget.selectedData:
            draw_tag(self, p_Widget.selectedData['Equipment Tags'], minX, minY, maxX, maxY, selected_location, selected_part,et_column_data, color=Qt.darkMagenta)
        if 'Sensors' in p_Widget.selectedData:
            draw_sensors(self, p_Widget.selectedData['Sensors'], minX, minY, maxX, maxY, selected_location, selected_part,ss_column_data, color=Qt.darkGray)
        #        if p_Widget.ckSensor.isChecked() and 'Sensors' in p_Widget.selectedData:
        #            draw_sensors(self, p_Widget.selectedData['Sensors'], minX, minY, maxX, maxY, color=Qt.black)
        if 'Area Breaks' in p_Widget.selectedData:
            draw_area_breaks(self, p_Widget.selectedData['Area Breaks'], minX, minY, maxX, maxY, selected_location,  selected_part, ab_column_data,  color=Qt.green)
